...
impl PlaybackQueue {
    ...
    pub fn load_tracks(&mut self, tracks: Vec<Track>) {
        ...
        let deduplicated: Vec<Track> = tracks
            .into_iter()
            .filter(|t| {
                // Allow selectable candidates (streamable + not geo/blocked), even if stream_url is missing
                if !crate::utils::track_filter::is_track_selectable(t) {
                    log::warn!(
                        "[Queue] Filtering out non-selectable track: {} (ID: {})",
                        t.title,
                        t.id
                    );
                    return false;
        ...
        log::info!(
            "[Queue] Loaded {} playable tracks (filtered non-playable)",
            deduplicated.len()
        );
    ...
    pub fn append_tracks(&mut self, tracks: Vec<Track>) {
        ...
        let existing_ids: std::collections::HashSet<u64> =
            self.original_tracks.iter().map(|t| t.id).collect();
        ...
        let new_tracks: Vec<Track> = tracks
            ...
            .filter(|t| {
                ...
                // Check if selectable (less strict than playable)
                if !crate::utils::track_filter::is_track_selectable(t) {
                    log::warn!(
                        "[Queue] Filtering out non-selectable track: {} (ID: {})",
                        t.title,
