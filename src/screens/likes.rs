...
fn render_liked_tracks(app: &mut MusicPlayerApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    match app.content.likes_sort_order {
        ...
        crate::app::player_app::LikesSortOrder::TitleAZ => {
            all_tracks_with_badges
                .sort_by(|a, b| a.0.title.to_lowercase().cmp(&b.0.title.to_lowercase()));
        }
        crate::app::player_app::LikesSortOrder::ArtistAZ => {
            all_tracks_with_badges.sort_by(|a, b| {
                a.0.user
                    .username
                    .to_lowercase()
                    .cmp(&b.0.user.username.to_lowercase())
            });
        }
    }
    ...
    ui.horizontal(|ui| {
        ...
        egui::ComboBox::from_id_salt("likes_sort")
            ...
            .show_ui(ui, |ui| {
                ui.selectable_value(
                    &mut app.content.likes_sort_order,
                    crate::app::player_app::LikesSortOrder::RecentFirst,
                    "Recent First",
                );
                ui.selectable_value(
...
fn render_tracks_grid_with_badges(
    app: &mut MusicPlayerApp,
    ui: &mut egui::Ui,
    _ctx: &egui::Context,
    tracks_with_badges: &[(crate::models::track::Track, &str)],
) {
    let card_size = 220.0;
    let spacing = 15.0;
    ...
    for row_tracks in tracks_with_badges.chunks(cards_per_row) {
        ui.horizontal(|ui| {
            ...
            for (track, badge) in row_tracks {
                // Render track card
                let (clicked, _shift_clicked, unlike_clicked) =
                    render_track_card_with_badge(app, ui, track, card_size, badge);
...
fn render_track_card_with_badge(
    ...
    // Returns (clicked, shift_clicked, unlike_clicked)
    ...
    if response.hovered() {
        ...
        ui.painter().rect_filled(rect, 8.0, BG_HOVER);
