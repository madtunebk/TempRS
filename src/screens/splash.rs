...
use crate::constants::{REPAINT_INTERVAL_CPU_ACTIVE, REPAINT_INTERVAL_GPU_MICROS};
use crate::utils::ShaderCallback;
use eframe::egui;

pub fn render_splash_screen(app: &mut MusicPlayerApp, ctx: &egui::Context) {
    let is_gpu = app.content.app_state.get_renderer_type() == crate::app_state::RendererType::Gpu;

    // Request repaint at mode-appropriate FPS
    if is_gpu {
        ctx.request_repaint_after(std::time::Duration::from_micros(
            REPAINT_INTERVAL_GPU_MICROS,
        ));
    } else {
        ctx.request_repaint_after(std::time::Duration::from_micros(
            REPAINT_INTERVAL_CPU_ACTIVE,
        ));
    }

    // Background: GPU → shader; CPU → solid color (no shader callbacks)
    if is_gpu {
        ...
        // CPU mode: simple dark background
        let rect = ctx.content_rect();
