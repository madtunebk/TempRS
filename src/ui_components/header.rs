...
fn render_navigation_icons(app: &mut MusicPlayerApp, ui: &mut egui::Ui) {
    ...
    let home_btn = ui
        .add_sized(
            ...
            egui::Button::new(egui::RichText::new("Home").size(14.0).color(home_color))
    ...
    let suggestions_color = if suggestions_active {
        ORANGE
    } else {
        LIGHT_GRAY
    };

    let suggestions_btn = ui
        .add_sized(
    ...
    let likes_btn = ui
        .add_sized(
            ...
            egui::Button::new(egui::RichText::new("Likes").size(14.0).color(likes_color))
    ...
    if app.ui.selected_tab == MainTab::Search
    ...
    if app.audio.current_track_id.is_some() {
        ...
        let icon_color = if now_playing_active {
            ORANGE
        } else {
            LIGHT_GRAY
        };
    ...
    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
        ...
        let logout_btn = ui
            .add_sized(
                ...
                egui::Button::new(egui::RichText::new("⏻").size(20.0).color(LIGHT_GRAY))
                    .fill(DARK_GRAY)
                    .corner_radius(CORNER_RADIUS),
            )
        ...
        if let Some(avatar_texture) = &app.auth.user_avatar_texture {
            ...
            ui.add(egui::Label::new(
                egui::RichText::new("●").size(20.0).color(LIGHT_GRAY),
            ));
        }
    });
}
