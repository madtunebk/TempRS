...
pub fn render_social_buttons(
    ...
    let unique_id = ui.id().with((
        "like",
        track_id,
        artwork_rect.min.x as i32,
        artwork_rect.min.y as i32,
    ));
    ...
    ui.painter()
        .circle_filled(like_rect.center(), button_size / 2.0, like_bg_color);
    ui.painter().text(
        like_rect.center(),
...
pub fn render_track_card(
    ...
    let (rect, response) =
        ui.allocate_exact_size(Vec2::new(card_size, full_height), Sense::click());
    ...
    if response.hovered() {
        ...
        ui.painter()
            .rect_filled(rect, 8.0, Color32::from_rgb(26, 26, 26));
    ...
    if let Some(texture) =
        crate::utils::artwork::load_track_artwork(app, &ctx, track.id, artwork_url, &cache_key)
    {
        ui.painter().image(
            texture.id(),
            artwork_rect,
        ...
        if let Some(url) = artwork_url {
            ...
            if let Some(no_artwork) = &app.ui.no_artwork_texture {
                ...
                ui.painter()
                    .rect_filled(artwork_rect, 6.0, Color32::from_rgb(55, 55, 60));
            }
        }
    }
    ...
    if response.hovered() {
        ...
        ui.painter().rect_filled(artwork_rect, 6.0, OVERLAY_DARK);
        ...
        ui.painter()
            .circle_filled(center, play_btn_radius, Color32::from_rgb(255, 85, 0));
...
pub fn calculate_grid_layout(available_width: f32, item_size: f32, spacing: f32) -> (usize, f32) {
    ...
    let items_per_row = ((usable_width + spacing) / (item_size + spacing))
        .floor()
        .max(1.0) as usize;
...
pub fn render_placeholder_card(ui: &mut egui::Ui, card_size: f32) {
    ...
    let full_height = card_size + metadata_height;
    let rect = ui
        .allocate_exact_size(Vec2::new(card_size, full_height), egui::Sense::hover())
        .0;

    // Grey placeholder box (matches artwork size only)
    let artwork_rect = egui::Rect::from_min_size(rect.min, Vec2::new(card_size, card_size));

    ui.painter()
        .rect_filled(artwork_rect, 6.0, Color32::from_rgb(50, 50, 55));
}
...
pub fn render_empty_state(ui: &mut egui::Ui, icon: &str, title: &str, message: &str) {
    ...
    ui.vertical_centered(|ui| {
        ...
        let (rect, _) =
            ui.allocate_exact_size(Vec2::new(card_width, card_height), egui::Sense::hover());

        // Draw card background
        ui.painter()
            .rect_filled(rect, 12.0, Color32::from_rgb(45, 45, 50));
        ui.painter().rect_stroke(
            rect,
            12.0,
    ...
    ui.horizontal(|ui| {
        ...
        ui.with_layout(egui::Layout::left_to_right(egui::Align::Center), |ui| {
            ...
            let prev_btn = ui.add_enabled(
                has_prev,
                egui::Button::new(egui::RichText::new("←").size(18.0).color(if has_prev {
                    Color32::WHITE
                } else {
                    Color32::DARK_GRAY
                }))
            ...
            let next_btn = ui.add_enabled(
                has_next,
                egui::Button::new(egui::RichText::new("→").size(18.0).color(if has_next {
                    Color32::WHITE
                } else {
                    Color32::DARK_GRAY
                }))
