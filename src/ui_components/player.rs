...
fn render_all_controls(app: &mut MusicPlayerApp, ui: &mut egui::Ui) {
    ...
    if play_btn.clicked() {
        log::info!(
            "[UI] Play/Pause clicked - current is_playing: {}",
...
fn render_volume_controls(app: &mut MusicPlayerApp, ui: &mut egui::Ui) {
    ...
    if app.ui.show_volume_popup {
        ...
        painter.rect_filled(popup_rect, 8.0, egui::Color32::from_rgb(35, 35, 40));
        painter.rect_stroke(
            popup_rect,
            8.0,
...
fn render_progress_bar(app: &mut MusicPlayerApp, ui: &mut egui::Ui) {
    ...
    ui.with_layout(Layout::left_to_right(egui::Align::Center), |ui| {
        ...
        let (response, painter) = ui.allocate_painter(egui::vec2(width.max(200.0), 20.0), sense);
        ...
        let (display_progress, is_dragging) = if can_seek && response.dragged() {
            if let Some(pos) = response.interact_pointer_pos() {
                ...
                app.ui.seek_target_pos = Some(std::time::Duration::from_secs_f32(
                    duration_secs * drag_progress,
                ));
        ...
        if is_dragging || app.ui.is_seeking {
            let current_rect = egui::Rect::from_min_max(
                bar_rect.min,
                egui::pos2(
                    bar_left + (bar_right - bar_left) * actual_progress,
                    bar_rect.max.y,
                ),
            );
        ...
        let progress_rect = egui::Rect::from_min_max(
            bar_rect.min,
            egui::pos2(
                bar_left + (bar_right - bar_left) * display_progress,
                bar_rect.max.y,
            ),
        );
        ...
        if response.clicked() {
            if let Some(pos) = response.interact_pointer_pos() {
                ...
                let seek_position = std::time::Duration::from_secs_f32(
                    (duration_secs * seek_progress).min(max_seek_secs),
                );
...
fn render_compact_social_buttons(app: &mut MusicPlayerApp, ui: &mut egui::Ui) {
    ...
    let like_btn = ui.add_enabled(
        has_track,
        egui::Button::new(egui::RichText::new(like_icon).size(16.0).color(like_color))
    ...
    if like_btn.hovered() {
        like_btn.on_hover_text(if app.is_current_track_liked() {
            "Unlike"
        } else {
            "Like"
        });
